...
SELECT EMPLOYEE_ID, CONCAT(FIRST_NAME, CONCAT(' ',  LAST_NAME)) 이름, UPPER(JOB_ID), DEPARTMENT_ID
    FROM EMPLOYEES
    WHERE UPPER(LAST_NAME) = 'KING';
    
--2. EMPLOYEES 테이블에서 King의 정보를 대문자로 검색하고 사원번호, 성명, 담당업무(대문자로),부서번호를 출력하라.
SELECT EMPLOYEE_ID, CONCAT(FIRST_NAME, CONCAT(' ',  LAST_NAME)) 이름, LOWER(JOB_ID), DEPARTMENT_ID
    FROM EMPLOYEES
...
SELECT LAST_NAME, INSTR(FIRST_NAME, 'e', 1) AS 위치
    FROM EMPLOYEES;
    
--7. EMPLOYEES 테이블에서 부서번호가 80인 사람의 급여를 30으로 나눈 나머지를 구하여 출력하라.
SELECT FIRST_NAME, MOD(SALARY, 30) 나머지
    FROM (SELECT *
                 FROM EMPLOYEES
...
SELECT HIRE_DATE, 
            ...
            TRUNC(MOD((SYSDATE - HIRE_DATE), 7)) || '일'  "근무일수(주, 일)"
FROM EMPLOYEES
ORDER BY  "근무일수(주, 일)" DESC;
